  ...
  group('Test When class', () {
    group('Test toString()', () {
      ...
      test('Should return 1 hour', () {
        expect(When(DateTime.now().subtract(Duration(hours: 1))).toString(),
            '1 hour ago');
      });
      ...
      group('Test toTime()', () {
        test('Should return 1 long year ago', () {
          var past = DateTime.now().subtract(Duration(days: 365));
          var time = When(past).toTime();
          var result = '${time.amount} long ${time.unit} ago';
          expect(result, '1 long year ago');
        });
      });
    });
  });
